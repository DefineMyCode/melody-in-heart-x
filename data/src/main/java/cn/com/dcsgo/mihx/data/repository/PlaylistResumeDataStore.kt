...
class PlaylistResumeDataStore(
    ...
    override fun currentSourcePlaylistId(): Int? = runBlocking(Dispatchers.IO) {
        store.data.map { preferences -> preferences[sourcePlaylistKey] }.first()
    }

    override suspend fun setSourcePlaylist(playlistId: Int?) {
        store.edit { preferences ->
            if (playlistId != null) {
                preferences[sourcePlaylistKey] = playlistId
            } else {
    ...
    override fun recordCurrentSourceBlocking(songId: Int): Boolean {
        val source = currentSourcePlaylistId() ?: return false
