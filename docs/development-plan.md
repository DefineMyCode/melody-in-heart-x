...
- [x] `EmbeddedLyricsReader`（`:data/lyrics`）：读取内嵌歌词（MP3 ID3v2 `USLT` / FLAC Vorbis `LYRICS`，仅读文件头部）；`LyricsRepositoryImpl` 优先外部 `.lrc`，回退内嵌。`LyricsViewModel` 对非同步歌词（无时间戳，`timeMs` 全 0）不做高亮/自动滚动、`onLineClick` 不误 seek。
